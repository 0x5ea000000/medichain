...
func TestUserQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetUserRequest{Index: msgs[0].Index},
			response: &types.QueryGetUserResponse{User: &msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetUserRequest{Index: msgs[1].Index},
			response: &types.QueryGetUserResponse{User: &msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetUserRequest{Index: "missing"},
